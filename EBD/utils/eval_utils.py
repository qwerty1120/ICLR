...
from utils.chem_utils import set_rdmol_positions, get_best_rmsd


def get_rmsd_confusion_matrix(data: Data, useFF=False):
    data['pos_ref'] = data['pos_ref'].reshape(-1, data['rdmol'].GetNumAtoms(), 3)
    data['sample_gen'] = data['sample_gen'].reshape(-1, data['rdmol'].GetNumAtoms(), 3)
    num_gen = data['sample_gen'].shape[0]
    num_ref = data['pos_ref'].shape[0]

    rmsd_confusion_mat = -1 * np.ones([num_ref, num_gen],dtype=np.float)
    
    for i in range(num_gen):
        gen_mol = set_rdmol_positions(data['rdmol'], data['sample_gen'][i])
        if useFF:
...
class CovMatEvaluator(object):
    ...
    def __call__(self, packed_data_list, start_idx=0):
        ...
        for data in packed_data_list:
            if 'sample_gen' not in data or 'pos_ref' not in data: continue
            ...
            if data['sample_gen'].shape[0] < num_gen: continue
            data['sample_gen'] = data['sample_gen'][:num_gen]
