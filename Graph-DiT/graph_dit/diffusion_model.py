...
class Graph_DiT(pl.LightningModule):
    ...
    def compute_laplacian_eigenvalues(self, data_batch):
        """
        주어진 DataBatch 객체(data_batch)에 대해 각 그래프마다 Laplacian 행렬의 고유값(eigenvalues)를 계산합니다.
        
        Args:
            data_batch (torch_geometric.data.Batch): 노드, edge_index, ptr, batch 등을 포함하는 Batch 객체.
            
        Returns:
            lap_eigs (list of np.ndarray): 각 그래프의 Laplacian 고유값 배열의 리스트.
        """
        # lap_eigs = []
        ...
        #     # 현재 그래프의 노드 인덱스(배치 내에서의 인덱스)
        #     node_indices = torch.arange(start, end, device=data_batch.x.device)
        ...
        #     # edge_attr는 Laplacian 계산에 필요하지 않으므로 생략
        #     A = to_scipy_sparse_matrix(sub_edge_index, num_nodes=num_nodes_sub)
        #     # 정규화된 Laplacian 계산 (normed=True)
        #     L = laplacian(A, normed=True)
        #     eigs = eigvalsh(L.toarray())
        #     lap_eigs.append(1/(np.max(eigs) - np.mean(eigs) + 1))
        lap_eigs = []
        num_graphs = data_batch.ptr.size(0) - 1  # 각 그래프의 개수
        ...
        for i in range(num_graphs):
            ...
            num_nodes_sub = end - start
            # 인접행렬 A 생성 (edge_attr는 고려하지 않고 연결성만 사용)
            A = to_scipy_sparse_matrix(sub_edge_index, num_nodes=num_nodes_sub)
            
            # normed=False → L = D - A
            L = laplacian(A, normed=False)
            
            # L을 밀집 배열로 변환한 후 고유값 계산 (오름차순 정렬)
            eigs = eigvalsh(L.toarray())
            # lap_eigs.append(4.3/(np.max(eigs) - np.mean(eigs) + 1))
            lap_eigs.append(np.max(eigs))
    ...
    def training_step(self, data, i):
        ...
        noisy_data = self.apply_noise(X, E, data.y, node_mask, eigenvalues=self.compute_laplacian_eigenvalues(data))
        # @@
    ...
    def apply_noise(self, X, E, y, node_mask, eigenvalues=None):
        ...
        if eigenvalues is not None:
            eigen_tensor = torch.tensor(eigenvalues, dtype=beta_t.dtype, device=beta_t.device).unsqueeze(1)
            eigen_tensor = eigen_tensor - eigen_tensor.min() + 1
            beta_t = beta_t * eigen_tensor  # element-wise 곱셈, shape: (bs, 1)
