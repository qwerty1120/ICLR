...
# @@
    def compute_uniqueness(self, valid):
        ...
        return unique, len(unique) / len(valid) if valid else 0.0

    def compute_novelty(self, unique):
        if self.dataset_smiles_list is None or len(unique) == 0:
            return [], 0.0
        train_set = set(self.dataset_smiles_list)
...
# @@
    def compute_relaxed_validity(self, generated, ensure_connected):
        ...
        for graph in generated:
            ...
            # # @@
            # mol = check_mol(mol, largest_connected_comp=True)
            # # check_mol 이 None 이면 완전히 실패한 거
            # if mol is None:
            #     all_smiles.append(None)
            #     continue
            # # 이 시점에서 mol 은 “가장 큰 연결 성분만 남긴” 분자입니다.
            # direct_valid_flag = True
            # direct_valid_count += 1
            direct_valid_flag = True if check_mol(mol, largest_connected_comp=False) is not None else False #lcc = True
    ...
    def evaluate(self, generated, targets, ensure_connected, active_atoms=None):
        ...
        if validity > 0: 
            ...
            # @@---- (1) uniqueness ----
            uniques, uniq_ratio = self.compute_uniqueness(valid)
            dist_metrics['uniqueness'] = uniq_ratio
            print(f"Uniqueness: {uniq_ratio*100:.2f}% ({len(uniques)}/{len(valid)})")

            # ---- (2) novelty  -------
            novel, nov_ratio = self.compute_novelty(uniques)
            dist_metrics['novelty'] = nov_ratio
            print(f"Novelty:   {nov_ratio*100:.2f}% ({len(novel)}/{len(uniques)})")

            print(f"Uniqueness over {len(valid)} valid molecules: {uniq_ratio*100:.2f}% "
                  f"({len(uniques)} unique SMILES)")
            print(f"Novelty over {len(uniques)} unique molecules: {nov_ratio*100:.2f}% "
                  f"({len(novel)} novel SMILES)")
            # @@
            ...
            targets_log = None
        # @@
        return unique, dict(nc_min=nc_min, nc_max=nc_max, nc_mu=nc_mu), all_smiles, dist_metrics, targets_log, uniques, novel
...
def compute_molecular_metrics(molecule_list, targets, train_smiles, stat_ref, dataset_info, task_evaluator, comput_config):
    ...
    all_smiles = evaluated_res[-5]
    all_metrics = evaluated_res[-4]
    targets_log = evaluated_res[-3]
    unique_smiles = evaluated_res[0]
    # @@
    novelty = evaluated_res[-1]
    unique = evaluated_res[-2]
    # @@

    return unique_smiles, all_smiles, all_metrics, targets_log, unique, novelty
